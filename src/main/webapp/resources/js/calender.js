...
function datePicker(tYear, tMonth, tDay, tYoil) { // 텍스트박스에 날짜 넣기 위해 만든 함수
	...
	selDate.value = picDate.getFullYear() + "년 " + (picDate.getMonth() + 1)
			+ "월 " + picDate.getDate() + "일" + "(" + tYoil + ")"
}
...
function calendar(tYear, tMonth) { // 달력 함수
	...
	var endDay = new Array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31); // 각달의
																			// 마지막
																			// 날짜
	var dayName = new Array("일", "월", "화", "수", "목", "금", "토"); // 숫자 요일을 문자 요일
																// 바꿀 함수
	...
	if ((eDate.getMonth() == 1)
			&& (((eDate.getYear() % 4 == 0) && (eDate.getYear() % 100 != 0)) || eDate
					.getYear() % 400 == 0)) {
		lastDay = 29;
	...
	calendarStr += "<font size=5color=black><a href=javascript:calendar("
			+ tYear + "," + (tMonth - 1) + ") class=preNext>◀</a></font>" // 월을
																			// 넘길때
																			// 빼기
																			// -1을
																			// 해서
	...
	calendarStr += "<font size=5 color=black>  <b>" + eDate.getFullYear()
			+ "년 " + (eDate.getMonth() + 1) + "월</b></font> "// 해당하는 년도와 월 표시
	calendarStr += "</TD><TD valign=middle>"
	calendarStr += "<font size=5 color=black><a href=javascript:calendar("
			+ tYear + "," + (tMonth + 1) + ") class=preNext>▶</a></font>" // 월을
																			// 넘길때
																			// 더하기
																			// +1을
	...
	for (i = 0; i < dayName.length; i++) {
		calendarStr += "<TD class=week><font size=5 color=black>" + dayName[i]
				+ "</font></TD>" // 숫자 요일을 날짜 요일로 입력
	}
	...
	for (i = 1; i <= lastDay; i++) { // 해당 월의 달력
		if (eDate.getFullYear() == nYear && eDate.getMonth() == nMonth
			...
			calendarStr += "<TD class=today onmouseover=onMouse(this) onmouseout=outMouse(this,'today') onClick=datePicker("
					...
					+ ",'"
					+ dayName[col]
					+ "')><font size=5 color=black>" + i + "</font></TD>"
		} else {
	...
	// calendarStr +="</TR><TR align=center><TD colspan=7 ><input name=selDate
	// class=selDate type=text readonly></TD></TR></TABLE>"
